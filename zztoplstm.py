# This code for the ZZTop-LSTM model
...
nameTopoFeat_0 = folderTOPO + '/TopoFeatures_dim0_win' + str(LBack) + '.csv'
nameTopoFeat_1 = folderTOPO + '/TopoFeatures_dim1_win' + str(LBack) + '.csv'
nameTopoFeat_ALL = folderTOPO + '/TopoFeatures_dimALL_win' + str(LBack) + '.csv'

df_TopoF_0 = pd.read_csv(nameTopoFeat_0) #
df_TopoF_1 = pd.read_csv(nameTopoFeat_1) #.iloc[:,selectTopoFeat]
df_TopoF_ALL = pd.read_csv(nameTopoFeat_ALL) #.iloc[:,selectTopoFeat]

# select topological features
# TOPO_F: BirthMean	BirthSTD DeathMean DeathSTD	LifeMean LifeSTD	NumBar	NumPrev	NumNext	DistQ0-Q4	CosSimQ0-Q4
# Feat2D = list(range(9,19))
TopoFeats =  [0,1,2,3,4,5,6,7,8] # + Feat2D] 
...
###################
# select features
df_F_0 = df_TopoF_0.iloc[:,TopoFeats]
df_F_1 = df_TopoF_1.iloc[:,TopoFeats]
df_F_A = df_TopoF_ALL.iloc[:,TopoFeats]
# select dim
df_Full_T = pd.concat([df_F_0, df_F_1, df_F_A], axis=1)
# df_Full_T = pd.concat([df_F_0, df_F_1], axis=1)
# df_Full_T = df_F_0
# df_Full_T = df_F_1
# df_Full_T = df_F_A
...
for ID_RUN in range(Number_Runs):
    ...
    df_T = df_Full_T.copy() # All variables
    # --------------- Regresor variables ...
    df_X = df_Full_X.copy() # All variables
    # --------------- Response variables ...
    df_Y = df_Full_Y.copy() # All variables
    # --- Features ...
    NTopoFeatures = df_T.shape[1] # Number of Topological Features (Input)
    ...
    df_T = df_T.head(df_X.shape[0] - LBack + 1)
    ...
    scaler_X.fit(df_X)
    scaler_Y.fit(df_Y)
    dataNorm_T = scaler_T.transform(df_T)
    ...
    dataX = np.concatenate([dataX, dataNorm_T], axis=1)
    ...
    model.add(LSTM(256, activation='linear', return_sequences=True, input_shape=(1, LBack*NFeatures + NTopoFeatures), dropout=pDrop))
